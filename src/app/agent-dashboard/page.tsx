...
export default async function AgentDashboardPage() {
  ...
  if (!canAccessPath(session.role, "/agent-dashboard")) redirect("/access-denied");
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        <div className="mb-8">
          ...
          <p className="mt-2 text-white/50">
            הנכסים שלך, גיוס לקוחות והגדרות פרופיל אישי.
          </p>
        </div>

        <DashboardWorkspace role={session.role} agentOnly />
      </main>
    </>
  );
}
